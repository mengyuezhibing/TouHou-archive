/**
 * TH06NC（东方红魔乡 新典版 / New Classic）「PKGL」归档格式
 *
 * ── 文件布局 ────────────────────────────────────────────────────
 *   [8 字节]   "PKGL" | u32 索引区字节数
 *   [索引区]   逐条记录，按 16 字节周期 XOR 加密
 *   [数据区]   各条目数据，偏移与大小由索引给出
 *
 * ── 索引记录（32 字节定长头 + 变长名字）────────────────────────
 *   u16  flags       bit0 = 数据是否经 zstd 压缩
 *   u32  seed        该条目数据的密钥种子
 *   u64  originalSize  解压后大小
 *   u64  storedSize    磁盘上占用大小
 *   u64  offset        数据在文件中的偏移
 *   u16  nameLength
 *   char name[nameLength]
 *
 * ── 加密 ────────────────────────────────────────────────────────
 *   索引区与数据区用的是**同一个** XOR 例程，区别只在种子：
 *     索引区种子 = CRC32(归档名去掉路径与扩展名)   例："th06MD" → 0x6c9807ba
 *     数据区种子 = 记录里的 seed 字段
 *   密钥由种子经 SplitMix64 派生：
 *     s   = ((seed × 0x9e3779b1) + 1) XOR (seed << 32)
 *     key = SplitMix64(s) ‖ SplitMix64(s + 0x9e3779b97f4a7c15)
 *   解密即 buf[i] ^= key[i % 16]。
 *
 *   算法来自对 th06nc.exe 的反汇编（函数 0x140059cb0 派生密钥、
 *   0x140059e20 计算归档名 CRC32、0x14005a511 为数据区调用点）。
 *
 * ── 校验 ────────────────────────────────────────────────────────
 *   7 个归档 / 1783 个条目全部解出，大小与索引声明逐字节吻合。
 */
import fs from 'node:fs';
import path from 'node:path';
import { decompress as zstdDecompress } from 'fzstd';

/** "PKGL" 的小端 u32 表示 */
export const PKGL_MAGIC = 0x49474b50; // 见下方 parseIndex：按小端读入后与 'PKGL' 比较

const GAMMA = 0x9e3779b97f4a7c15n;
const MIX1 = 0xbf58476d1ce4e5b9n;
const MIX2 = 0x94d049bb133111ebn;
const MASK64 = (1n << 64n) - 1n;
/** 种子混合用的小常量，与 x64 立即数 0x9e3779b1 对应 */
const SEED_MUL = 0x9e3779b1n;

// ------------------------------------------------------------ CRC32

/** 标准 CRC-32（IEEE 802.3，多项式 0xEDB88320）查表 */
const CRC_TABLE = (() => {
  const t = new Int32Array(256);
  for (let n = 0; n < 256; n++) {
    let c = n;
    for (let k = 0; k < 8; k++) c = c & 1 ? 0xedb88320 ^ (c >>> 1) : c >>> 1;
    t[n] = c;
  }
  return t;
})();

/** 计算 CRC32，返回无符号 32 位值 */
export function crc32(buf: Buffer | Uint8Array | string): number {
  const data = typeof buf === 'string' ? Buffer.from(buf, 'latin1') : buf;
  let c = 0xffffffff;
  for (let i = 0; i < data.length; i++) c = CRC_TABLE[(c ^ data[i]) & 0xff] ^ (c >>> 8);
  return (c ^ 0xffffffff) >>> 0;
}

// ------------------------------------------------------------ 密钥派生

function splitMix(z: bigint): bigint {
  z = ((z ^ (z >> 30n)) * MIX1) & MASK64;
  z = ((z ^ (z >> 27n)) * MIX2) & MASK64;
  return z ^ (z >> 31n);
}

/**
 * 由 32 位种子派生 16 字节 XOR 密钥。
 *
 * 对应反汇编里的一段：先 `seed * 0x9e3779b1 + 1`，再与 `seed << 32` 异或，
 * 得到 SplitMix64 的初始状态；随后连续取两个输出，各 8 字节小端拼成密钥。
 */
export function deriveKey(seed32: number): Buffer {
  const seed = BigInt(seed32 >>> 0);
  let s = ((seed * SEED_MUL) & MASK64) + 1n;
  s &= MASK64;
  s ^= (seed << 32n) & MASK64;

  const k1 = splitMix((s + GAMMA) & MASK64);
  const k2 = splitMix((s + 2n * GAMMA) & MASK64);

  const out = Buffer.allocUnsafe(16);
  out.writeBigUInt64LE(k1, 0);
  out.writeBigUInt64LE(k2, 8);
  return out;
}

/**
 * 归档名的密钥种子。
 *
 * 取路径的最后一段并去掉扩展名（`data\th06CM.dat` → `th06CM`），
 * 与原程序 0x140059e20 的行为一致：它先找最后一个 `\` 或 `/`，
 * 再找最后一个 `.`，只对中间那段算 CRC32。
 */
export function archiveSeed(archivePathOrName: string): number {
  const base = archivePathOrName.replace(/\\/g, '/').split('/').pop() ?? '';
  const dot = base.lastIndexOf('.');
  const stem = dot > 0 ? base.slice(0, dot) : base;
  return crc32(stem);
}

/** 对缓冲区做周期性 XOR 解密（原地修改并返回） */
export function xorDecrypt(buf: Buffer, key: Buffer): Buffer {
  for (let i = 0; i < buf.length; i++) buf[i] ^= key[i & 15];
  return buf;
}

// ------------------------------------------------------------ 索引

export interface PkgEntry {
  /** 标志位，bit0 表示数据经 zstd 压缩 */
  flags: number;
  /** 数据区密钥种子 */
  seed: number;
  /** 解压后的原始大小 */
  originalSize: number;
  /** 磁盘上的存储大小 */
  storedSize: number;
  /** 数据在归档文件中的偏移 */
  offset: number;
  name: string;
}

/** 记录头定长部分：u16 flags + u32 seed + u64×3 + u16 nameLen */
const RECORD_HEADER_SIZE = 32;

export function parseIndex(indexBuf: Buffer): PkgEntry[] {
  const entries: PkgEntry[] = [];
  let off = 0;

  while (off + RECORD_HEADER_SIZE <= indexBuf.length) {
    const flags = indexBuf.readUInt16LE(off);
    const seed = indexBuf.readUInt32LE(off + 2);
    const originalSize = Number(indexBuf.readBigUInt64LE(off + 6));
    const storedSize = Number(indexBuf.readBigUInt64LE(off + 14));
    const offset = Number(indexBuf.readBigUInt64LE(off + 22));
    const nameLength = indexBuf.readUInt16LE(off + 30);

    if (nameLength === 0 || off + RECORD_HEADER_SIZE + nameLength > indexBuf.length) {
      throw new Error(`索引记录损坏：偏移 ${off} 处名字长度为 ${nameLength}`);
    }
    const name = indexBuf.toString('latin1', off + RECORD_HEADER_SIZE, off + RECORD_HEADER_SIZE + nameLength);
    entries.push({ flags, seed, originalSize, storedSize, offset, name });
    off += RECORD_HEADER_SIZE + nameLength;
  }
  return entries;
}

// ------------------------------------------------------------ 归档读取

/** 解密并（必要时）解压单个条目数据 */
export function decodeEntry(stored: Buffer, seed: number, compressed: boolean): Buffer {
  const plain = xorDecrypt(stored, deriveKey(seed));
  if (!compressed) return plain;
  const out = zstdDecompress(new Uint8Array(plain));
  return Buffer.from(out.buffer, out.byteOffset, out.byteLength);
}

/**
 * 已打开的 PKGL 归档。
 *
 * 索引一次性读入内存（最大 29KB），条目数据按需读取，
 * 因此打开 267MB 的 th06TL.dat 也只占几 KB。
 */
export class PkgArchive {
  readonly filePath: string;
  readonly entries: PkgEntry[];
  private readonly fd: number;
  private readonly size: number;

  private constructor(filePath: string, fd: number, size: number, entries: PkgEntry[]) {
    this.filePath = filePath;
    this.fd = fd;
    this.size = size;
    this.entries = entries;
  }

  static open(filePath: string): PkgArchive {
    const fd = fs.openSync(filePath, 'r');
    try {
      const size = fs.fstatSync(fd).size;
      const header = Buffer.alloc(8);
      fs.readSync(fd, header, 0, 8, 0);
      const magic = header.toString('latin1', 0, 4);
      if (magic !== 'PKGL') {
        throw new Error(`${path.basename(filePath)} 不是 PKGL 归档（魔数 ${JSON.stringify(magic)}）`);
      }
      const indexLength = header.readUInt32LE(4);
      if (indexLength === 0 || 8 + indexLength > size) {
        throw new Error(`索引长度 ${indexLength} 超出文件范围（文件 ${size} 字节）`);
      }

      const indexBuf = Buffer.alloc(indexLength);
      fs.readSync(fd, indexBuf, 0, indexLength, 8);
      // 索引区种子来自归档名，而非文件内任何字段
      xorDecrypt(indexBuf, deriveKey(archiveSeed(filePath)));

      return new PkgArchive(filePath, fd, size, parseIndex(indexBuf));
    } catch (err) {
      fs.closeSync(fd);
      throw err;
    }
  }

  /**
   * 读出并解码一个条目。
   * `noDecompress` 用于排查——此时压缩条目返回的是解密后的 zstd 字节流。
   */
  read(entry: PkgEntry, noDecompress = false): Buffer {
    if (entry.offset + entry.storedSize > this.size) {
      throw new Error(`条目 ${entry.name} 超出文件范围`);
    }
    const stored = Buffer.alloc(entry.storedSize);
    if (entry.storedSize > 0) {
      fs.readSync(this.fd, stored, 0, entry.storedSize, entry.offset);
    }
    const compressed = (entry.flags & 1) !== 0 && !noDecompress;
    return decodeEntry(stored, entry.seed, compressed);
  }

  /** 按名字查找并解码 */
  readByName(name: string): Buffer | null {
    const e = this.entries.find((x) => x.name === name);
    return e ? this.read(e) : null;
  }

  close(): void {
    fs.closeSync(this.fd);
  }

  /** 便捷方法：打开 → 回调 → 必定关闭 */
  static with<T>(filePath: string, fn: (a: PkgArchive) => T): T {
    const a = PkgArchive.open(filePath);
    try {
      return fn(a);
    } finally {
      a.close();
    }
  }
}
